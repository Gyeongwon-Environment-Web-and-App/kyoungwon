...
const VehicleCard: React.FC<VehicleCardProps> = ({
  ...
  return (
    <div className={`border ${status === 'broken' ? 'border-red' : 'border-d9d9d9'} rounded-lg flex py-6 px-4`}>
      <div className='flex flex-col justify-center'>
        ...
        <img src={getTruckIcon(teams?.category, size)} alt="트럭 아이콘" className='p-4 max-w-48' />
      </div>
      <div className="flex items-center">
        <div className={`border-l px-2 py-16 ${status === 'broken' ? 'border-red' : 'border-d9d9d9'}`}></div>
        <div className="flex flex-col gap-2">
          <p className="text-xl font-bold">{truckNum}</p>
          <div className="flex items-start gap-1">
            <img src={team} alt="기사 아이콘" className='w-6 h-6'/>
